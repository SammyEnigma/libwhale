...
namespace Whale
{
	namespace Crypto
	{
		namespace HMd5
		{
			...
			{
				return md5sum((const char *)content.data(), content.size());
			}
			...
			std::string md5sum(const std::string &fname)
			{
				...
				int err = md5_stream(fp, bin_buffer);
				if (0 != err) { fclose(fp); return ""; }
			...
			std::string md5sum(const char *buffer, size_t len)
			{
				size_t digest_hex_bytes = DIGEST_HEX_BYTES;
				unsigned char bin_buffer_unaligned[DIGEST_BIN_BYTES + DIGEST_ALIGN];
				/* Make sure bin_buffer is properly aligned. */
				...
				void *err = md5_buffer(buffer, len, bin_buffer);
