...
namespace Whale
{
	namespace Crypto
	{
		namespace HSha
		{
			...
			{
				Whale::Basic::HByteArray hash(32);
				picosha2::hash256(content.begin(), content.end(), hash.begin(), hash.end());
				return hash.toHex();
			}
			...
			bool verifyFile(const std::string& filePath, const std::string& hashVal)
			{
				...
				auto first = std::istreambuf_iterator<char>(ifs);
				auto last = std::istreambuf_iterator<char>();
				return picosha2::hash256_hex_string(first, last) == hashVal;
			}
		}
	}
}